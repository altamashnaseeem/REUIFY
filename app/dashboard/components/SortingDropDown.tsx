...
//save to LocalStorage whenever soringOptions changes
useEffect(()=>{
...
useEffect(()=>{
    ...
    const sorted=sortProjects(allProjects,selectedOption?.options.find((option)=>option.selected)?.value as string);
    setSortedProjects(sorted);
},[allProjects])  
      ...
      {sortingOptions.map((category,categoryIndex)=>(
          ...
          <div className='flex flex-col gap-2 ml-2 mt-[2px]'>
             ...
             {category.options.map((option,optionIndex)=>(
                 <div key={optionIndex} onClick={()=>handleOptionClick(categoryIndex,optionIndex)}>
                 <span  className={`${option.selected ? "text-sky-500":""} cursor-pointer hover:text-sky-500`}>{option.label}</span>
             </div>
